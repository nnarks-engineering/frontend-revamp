      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6 scrollbar-hide">
        {messages.length === 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center px-4 max-w-xl mx-auto">
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-primary/15 via-primary/5 to-secondary/15 flex items-center justify-center mb-6 border border-white">
              <Bot className="w-10 h-10 text-primary" />
            </div>
            <h3 className="text-2xl font-semibold text-foreground mb-3 font-millik">
            ...
            {messages.map((msg) => (
              ...
              >
                ...
                <div className={cn(
                  ...
                  msg.role === "assistant"
                    ? "bg-gradient-to-br from-primary/20 to-secondary/20 border border-white"
                    : "bg-muted"
                )}>
                  {msg.role === "assistant" ? (
    ...
    <div className="h-full flex flex-col bg-white border-l border-border/40 w-80 shrink-0">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide py-2 px-3 space-y-0.5">
        <div className="flex flex-col items-center justify-center h-40 text-center px-4">
          ...
          <p className="text-[12px] text-muted-foreground font-medium">No recent chats</p>
          <p className="text-[11px] text-muted-foreground/60 mt-1">Start a conversation to see your history here.</p>
        </div>
      </div>
    </div>
  );
...
export const Route = createFileRoute('/_app/inbox/ai')({
